...
- (void)moveToViewNumber:(NSInteger)viewNumber
{
    ...
    [self updateColorsForButtonIndex:viewNumber];
...
- (void)connectButtons
{
    ...
    if (_pageMode == MBX_SegmentController) {
        ...
        [segmentController addTarget:self action:@selector(segmentControllerMode:) forControlEvents:UIControlEventValueChanged];
    ...
    for (UIButton* button in buttons){
        ...
        switch (_pageMode) {
            case MBX_FreeButtons:
                [self updateColorsForButtonIndex:0];
...
- (void)freeButtonsMode:(UIButton *)button
{
    @try {
        ...
        [self updateColorsForButtonIndex:destination];
    }
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(UIViewController *)viewController
{
    ...
    [self updateColorsForButtonIndex:index];
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed
{
    if (completed) {
        ...
        [self MBXPageChangedToIndex:_currentPageIndex];
        [self updateColorsForButtonIndex:_currentPageIndex];
    }
}
...
- (void) updateColorsForButtonIndex:(NSInteger)index {
    NSLog(@"Index : %ld", index);
    for (UIButton *  button in  [[self MBXDataSource] MBXPageButtons]) {
        
        if (button.tag == index) {
            [button setSelected:YES];
            [ReusedMethods applyBottomBorderIndicator:button];
        }
        else{
            [button setSelected:NO];
            [ReusedMethods removeBottomBorder:button];
        }
    }
}
